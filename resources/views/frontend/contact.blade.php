    ...
    <body>
        <section class="pt-20">
            ...
            <div class="flex">
                ...
                <div class="w-1/4 bg-dark text-white p-8">
                    <h1 class="text-2xl font-bold text-center font-montserrat mt-4">Contact Center</h1>
                    <img src="{{asset('storage/'.$contacts->cover)}}" alt="Profile Picture"
                        class="mx-auto my-10 rounded-full shadow-2xl w-40 h-40 object-cover border-4 border-white">
                    <h1 class="text-2xl font-bold text-center mt-4">{{$contacts -> contact_name}}</h1>
                    <ul class="mt-8">
                        <li class="mb-4">
                ...
                <div class="w-3/4 p-8 pl-20 pr-20">
                    ...
                    <form action="{{ route('orderticket.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            <label for="package" class="text-lg">Package:</label>
                            <select id="package" name="package"
                                class="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                                onchange="updatePrice()">
                                @foreach ($packages as $package)
                                <option value="{{ $package->packages_id }}" data-price="{{ $package->rate }}">
                                    {{ $package->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <button type="submit"
                            class="bg-black text-white px-6 py-3 rounded-md hover:bg-primary transition animate-bounce">Buy
                            Now</button>
                    </form>
                </div>
